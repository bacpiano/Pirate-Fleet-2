    ...
    func addPlayerShipsMinesMonsters(numberOfMines: Int = 0, numberOfSeamonsters: Int = 0) {
        ...
        for (requiredShipType, requiredNumber) in Settings.RequiredShips {
            for _ in 0..<requiredNumber {
                ...
                // Change initializer method by removing hitTracker because it has defualt value in the initializer method
        ...
        for _ in 0..<numberOfMines {
            ...
            var mine = Mine(location: location  ,penaltyText: "Mine hit!!!" , guaranteesHit: true)
            while !gridViewController.addMine(mine, playerType: .Computer) {
                location = RandomGridLocation()
                mine = Mine(location: location,penaltyText: "Mine hit!!!",guaranteesHit: true )
            }
        }
        ...
        for _ in 0..<numberOfSeamonsters {
            var location = RandomGridLocation()
            var seaMonster = SeaMonster(location: location, guaranteesHit: true ,penaltyText: "Sea monster has been awaken !!!")
            while !gridViewController.addSeamonster(seaMonster, playerType: .Computer) {
                location = RandomGridLocation()
                seaMonster = SeaMonster(location: location, guaranteesHit: true ,penaltyText: "Sea monster has been awaken")
            }
        }
    }
}